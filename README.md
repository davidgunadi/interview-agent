...
**Version:** 2.1.2 — see [CHANGELOG.md](CHANGELOG.md) for what changed and why.
...
Run the review skill with the candidate name. The recording name is optional — if omitted, Claude searches Fireflies for recordings containing the candidate name and picks the right one (asking you to confirm if there are multiple matches). Requires the Fireflies connector to be authorized.

```
/review-interview [candidate-name]
...
- Fireflies integration: recording name is optional when running `/review-interview` — omit it to auto-search by candidate name, or provide a date/title to target a specific recording; requires the Fireflies connector to be authorized
